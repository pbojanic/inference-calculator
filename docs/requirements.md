...
The query is mapped to the HuggingFace models API by its shape. When the input contains a `/`, it is treated as **org-scoped**: the part before the first slash becomes the `author=` filter (an exact namespace match) and any text after the slash becomes the `search=` term, so `mistralai/` returns that org's models and `mistralai/Mistral-Small` narrows within it. A slash-less query is a plain full-text `search=`. Org-scoped searches deliberately **omit** the `text-generation` pipeline filter, because many current LLMs are tagged otherwise (e.g. `mistralai/Mistral-Small-3.2-24B-Instruct-2506` is tagged `image-text-to-text`) and would otherwise be silently hidden when the user has explicitly named the org; slash-less full-text searches keep the `text-generation` filter to stay relevant.
